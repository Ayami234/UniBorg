...
async def _(event):
    ...
    if reply_message.video or reply_message.gif:
        file = await borg.download_media(reply_message, thumb=-1, progress_callback = lambda current, total: event.edit(f'Downloaded {current} out of {total} ' + 'bytes: {:.2%}'.format(current / total))
    else:
